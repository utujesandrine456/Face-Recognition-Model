...
def resolve_embedder_model_path(model_path: str | Path = DEFAULT_EMBEDDER_MODEL) -> str:
    """Return a usable ArcFace ONNX path, falling back from the empty placeholder."""
    requested = Path(model_path)
    fallback = FALLBACK_EMBEDDER_MODEL

    if requested.exists() and requested.stat().st_size > 0:
        return str(requested)

    if requested == DEFAULT_EMBEDDER_MODEL and fallback.exists() and fallback.stat().st_size > 0:
        return str(fallback)

    if requested.exists() and requested.stat().st_size == 0:
        raise RuntimeError(
            f"ArcFace model file is empty: {requested}. "
            f"Replace it with a real ONNX model or use {fallback}."
        )

    raise FileNotFoundError(
        f"ArcFace model not found: {requested}. "
        f"Expected a real ONNX model at {requested} or {fallback}."
    )
...
def draw_embedding_matrix(
    img: np.ndarray, emb: np.ndarray, top_left=(10, 220), cell_scale: int = 6, title: str = "embedding"
):
    D = emb.size
    ...
    heat = cv2.resize(
        heat, (cols * cell_scale, rows * cell_scale), interpolation=cv2.INTER_NEAREST,
    )
    ...
    while True:
        ...
        if faces:
            ...
            ww, hh = draw_embedding_matrix(
                vis, res.embedding, top_left=(HEAT_X, HEAT_Y), cell_scale=CELL_SCALE, title="embedding heatmap",
            )
            if ww > 0:
                cv2.putText(
                    vis, emb_preview_str(res.embedding), (HEAT_X, HEAT_Y + hh + 28),
